...
function makeEvent(tool: TraceEvent["tool"], ts: string): TraceEvent {
    return { type: "tool", session: "test", ts, tool };
}
...
describe("TimelineView", () => {
    ...
    it("agrupa eventos en prompts separados por gaps de más de 60 segundos", async () => {
        const events = [
            makeEvent("okf_traverse", "2026-07-19T04:00:00.000Z"),
            makeEvent("okf_read", "2026-07-19T04:00:01.000Z"),
            makeEvent("okf_search", "2026-07-19T04:02:00.000Z"),
        ];
    ...
    it("expone progreso y controles de pausa y detención del replay", async () => {
        ...
        const onStop = vi.fn();
        const onPause = vi.fn();
        const events = [makeEvent("okf_traverse", "2026-07-19T04:00:00.000Z")];
        const { root, view } = makeView(events, {
            activate: onActivate,
            stop: onStop,
            pause: onPause,
